...
describe('Create Python process and run it', () => {
  it('creates python process and runs', () => {
    ...
    cy.get('form select.form-control.form-control-sm').select('Python');
    cy.get('form > :nth-child(1) > :nth-child(4)').type('h_test.py');

    cy.get('.view-lines').invoke('text', "\nprint('hello world!')");
 
    cy.get('.modal-footer').contains('Add').click();

    cy.get('ul#process_folder_python_target').contains('h_test.py').click();

    cy.get('#processid').then(($input) => {
      const processId = $input.val(); 
      console.log('process id ',processId)
      const selector = `[onclick="GW.process.runProcess('${processId}', 'h_test.py', 'python')"]`;
  ...
  //   cy.get('#add-process-206').click();
  //   cy.get('#process-xd04rh > .row > .col-md-8').should('be.visible');
  //   /* ==== End Cypress Studio ==== */
  // });
});
